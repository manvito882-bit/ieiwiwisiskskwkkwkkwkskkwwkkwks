-- Удаляем старые функции и создаем новые с упрощенным хешированием
DROP FUNCTION IF EXISTS public.create_user_with_username(text, text, boolean);
DROP FUNCTION IF EXISTS public.authenticate_user(text, text);

-- Создаем функцию для создания пользователя с простым хешированием
...
BEGIN
  ...
  -- Создаем пользователя через auth с простым хешированием
...
-- Создаем функцию для аутентификации с простым хешированием
...
BEGIN
  ...
  -- Создаем временный email и хеш пароля
  temp_email := username_param || '@temp.local';
  password_hash := encode(sha256(password_param::bytea), 'hex');

  -- Проверяем пароль через auth.users
  IF EXISTS (
    SELECT 1 FROM auth.users 
    WHERE email = temp_email 
    AND encrypted_password = password_hash
  ) THEN
